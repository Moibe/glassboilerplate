...
async function verificaSuRegistro(usuario) {
  ...
  //const q = query(colRef, where("document().ido", "==", "mtSbM39Ab9diXUbifERB"))
  ...
  //ESPACIO PARA FUNCION QUE BUSCA TAGS:
  async function buscaTags() { 

    const colTags = collection(db, 'onboarding-tags');

    //Query
    const a = query(colTags, where("visibility", "==", true));
    //Todo
    const q = query(colTags);

    await getDocs(q).then(querySnapshot => {

      //La cantidad de tags es: 
      const cuantos = querySnapshot.docs.length

      for (let i = 0; i < cuantos; i++) {
  
        const documentSnap = querySnapshot.docs[i];
        const documentId = documentSnap.id;
        console.log(documentId)
        //const docRef = doc(db, 'tags', documentId);
        //console.log("Y el docRef es: ", docRef)

        //obtenNombreTag(docRef).then(resultado => {
        //console.log("Nombre Tag:", resultado)
        //textoContainer.textContent = resultado;
        //suscribeteACambiosUsuario(docRefFinal);
      //})
...
async function obtenNombreTag(docRef){
  ...
  await getDoc(docRef)
    .then(doc => {
      tag = doc.data().arreglo[4]
      console.log("el nombre del tag (arreglo) es:", tag)
      })
    return tag
}
